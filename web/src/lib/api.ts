...
async function req<T>(method: string, path: string, body?: unknown): Promise<T> {
  const headers: Record<string, string> = { 'Content-Type': 'application/json', ...deviceHeaders() }; // + aparelho (lib/device.ts)
  const t = token.get();
...
export const api = {
  // auth
  register: (b: { nick: string; email: string; password: string; teamSlug: string; gender: string; ref?: string; elapsedMs?: number; turnstileToken?: string; origem?: Record<string, unknown> }) => req<{ token: string; me: Me }>('POST', '/api/auth/register', b),
