...
const ITEMS: { id: Scope; label: string }[] = [
  ...
  { id: 'penal', label: 'Pênalti' }, { id: 'falta', label: 'Falta' }, { id: 'trilha', label: 'Trilha' }, { id: 'futprego', label: 'Ranking X1' },
];
...
export function RankingsScreen() {
  ...
  const sub = scope === 'hora' && key ? `HORA ${hourLabel(key)}` : scope === 'rodada' ? `RODADA ${key ?? ''}` : scope === 'temporada' ? `TEMPORADA ${key ?? ''}` : scope === 'geral' ? 'TODOS OS TEMPOS' : scope === 'futprego' ? 'PONTOS · TODOS OS TEMPOS' : `GOLS DE ${ITEMS.find((i) => i.id === scope)?.label.toUpperCase()}`;
  const fpPts = meta?.futprego?.points ?? { win: 3, draw: 1, loss: -2 };

  return (
    <div className="flex flex-col gap-3">
      <div className="flex justify-center"><div className="ribbon ribbon-orange ribbon-lg"><img src="/ui/ico-trophy_s.png" className="mr-2 h-9 w-9" alt="" />{scope === 'futprego' ? 'RANKING X1' : 'ARTILHARIA'}</div></div>
      <Tabs value={scope} onChange={pick} items={ITEMS} />
      <Panel title={sub} ribbon="blue">
        {rows === null ? <div className="flex justify-center py-8"><Spinner /></div> : <TopList rows={rows} highlight={me.nick} empty={scope === 'futprego' ? 'Ninguém pontuou no Ranking X1 ainda. Desafie alguém no FutPrego!' : 'Ninguém pontuou aqui ainda. Vai lá e chuta!'} />}
      </Panel>
      {scope === 'futprego'
        ? <div className="panel-full text-center text-[12px] font-extrabold">Ranking X1 = FutPrego 1x1. Vitória {fpPts.win > 0 ? '+' : ''}{fpPts.win} · empate {fpPts.draw > 0 ? '+' : ''}{fpPts.draw} · derrota {fpPts.loss}. Só partidas de verdade (treino contra bot e W.O. antes de 2 jogadas não contam). "Sem perder" = vitórias e empates seguidos.</div>
