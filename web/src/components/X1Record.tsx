 ...
 * a temporada (vitórias que contam no Ranking do X1 e a posição), o total de sempre — vitórias, derrotas,
 * empates e o aproveitamento como no futebol (3 pontos por vitória, 1 por empate) — e cada jogo. Sem
 * partidas: no próprio perfil, convite para jogar.
 */
  ...
  return (
    <Panel title="X1" ribbon="orange">
      {games === 0 ? (
        ...
        <>
          {season.number !== null && (
            <Link to="/rankings?aba=x1" className="mb-2 flex items-center gap-2 rounded-xl bg-gold/20 px-2 py-1.5 ring-1 ring-gold/60">
              ...
              <span className="min-w-0 flex-1 leading-tight">
                <span className="block text-[12px] font-extrabold text-muted">Temporada {season.number}</span>
                <span className="block text-[14px] font-extrabold text-navy-ink">
                  {season.wins ? `${season.wins} ${season.wins === 1 ? 'vitória' : 'vitórias'} no Ranking do X1` : 'Nenhuma vitória no Ranking do X1 ainda'}
                </span>
              </span>
            </Link>
          )}
          ...
          <ul className="mt-2 flex flex-col gap-1">
            {GAMES.map((g) => {
              const t = record.games?.[g.id] ?? { wins: 0, losses: 0, draws: 0 };
              return (
                <li key={g.id} className="flex items-center justify-between rounded-xl bg-sky/10 px-2 py-1">
                  ...
                  <span className="font-display text-[14px] tabular-nums">
                    <span className="text-grass-deep">{t.wins}V</span> <span className="text-navy-ink">{t.draws}E</span> <span className="text-danger">{t.losses}D</span>
                  </span>
                </li>
              );
            })}
