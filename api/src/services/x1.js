...
/**
 * Campanha de um jogador (perfil): todos os tempos (V/E/D, pontos e sequências), cada jogo do X1 (FutPrego e
...
export async function x1Ranking({ from = null, to = null, table = null, take = 50 } = {}, db = prisma) {
  const rows = await db.x1Match.findMany({ where: { ...X1_COUNTED, ...x1Period(from, to) }, orderBy: { id: 'asc' }, select: { aId: true, bId: true, winnerId: true } });
