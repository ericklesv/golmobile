...
export function RankingsScreen() {
  ...
  // ?aba=x1 (ou x1-rodada / x1-temporada / x1-geral) abre direto no Ranking X1 (links da tela do X1 e do perfil)
  const aba = params.get('aba') ?? '';
  const [scope, setScope] = useState<Scope>(() => (aba.startsWith('x1') ? 'x1' : ITEMS.some((i) => i.id === aba) ? (aba as Scope) : 'hora'));
  const [x1, setX1] = useState<X1Scope>(() => (X1_ITEMS.some((i) => i.id === aba) ? (aba as X1Scope) : 'x1-rodada')); // recorte dentro da aba Ranking X1
    ...
    <div className="flex flex-col gap-3">
      ...
      {scope === 'x1' ? (
        <div className="panel-full text-center text-[12px] font-extrabold">
          ...
          <p>Ranking X1 = os jogos 1x1 do X1 (FutPrego e Futebol de Botão). Vitória {fpPts.win > 0 ? '+' : ''}{fpPts.win} · empate {fpPts.draw > 0 ? '+' : ''}{fpPts.draw} · derrota {fpPts.loss}. Só partidas de verdade (treino contra bot e W.O. antes de 2 jogadas não contam); a partida conta no período em que terminou. "Sem perder" = vitórias e empates seguidos.</p>
        </div>
