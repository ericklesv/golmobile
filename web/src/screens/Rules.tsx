    ...
    <div className="flex flex-col gap-4">
      ...
      <Panel title="OS CHUTES" ribbon="orange">
        <ul className="flex flex-col gap-2 text-[13px] font-bold text-navy-ink">
          ...
          <li><b className="text-gold-deep">Pênalti</b> — escolha esquerda, meio ou direita. O goleiro pula pra um canto. +{fmt(meta?.money.PENALTY ?? 40)}.</li>
          <li><b className="text-sky-deep">Falta</b> — por fora da barreira ou por cima. +{fmt(meta?.money.FOUL ?? 90)}.</li>
          <li><b className="text-orange-deep">Trilha</b> — drible a defesa, o meio e o ataque tocando em um jogador por linha. {thieves ? `Roubam a bola: ${thieves}.` : 'Em cada linha, alguém rouba a bola.'} +{fmt(meta?.money.TRAIL ?? 160)}.</li>
          <li><b className="text-gold-deep">Party GoL</b> — a roleta: aposta {fmt(meta?.money.PARTY_BET ?? 100)} e 3 das 8 casas pagam, cada uma um valor: {(meta?.partyPrizes ?? [300, 800, 1500]).filter((p) => p > 0).map((p) => fmt(p)).join(', ')}. 10 giros por dia; a primeira vitória do dia vale gol.</li>
          <li><b className="text-grass-deep">Saldo nos minigames</b> — cada vitória paga além do gol, e <b>quanto mais difícil o minigame, mais ele paga</b>: o Alvo no Gol é o que mais dá, a Memória o que menos. O valor de cada um aparece no card dele, na tela inicial.</li>
          ...
          <li><b className="text-orange-deep">X1</b> — um jogo 1x1 ao vivo por dia, que troca às {meta?.x1?.today?.switchHour ?? 19}h, junto com a rodada: <b>FutPrego</b> (futebol de prego, uma vez de cada: arraste para trás e solte, e a bola quica nos pregos; sem gol em {meta?.futprego?.maxTurns ?? 10} jogadas de cada, o dinheiro volta) e <b>Futebol de Botão</b> (na sua vez, {meta?.x1?.botao.snapsPerTurn ?? 2} petelecos num botão seu; o primeiro gol acaba a partida; sem gol em {meta?.x1?.botao.maxTurns ?? 9} vezes, pênaltis). Quem desafia espera alguém aceitar (quem está nas telas do jogo recebe o convite). Cada um põe {fmt(meta?.futprego?.bet ?? 200)}; quem vencer leva {fmt((meta?.futprego?.bet ?? 200) * 2)} e 1 gol para o time, e o time do outro perde 1 gol na rodada. Só as {meta?.futprego?.maxGoalsPerHour ?? 10} primeiras partidas de cada jogador em cada hora mexem no placar (empate também conta); depois, até a hora virar, vale só o dinheiro. Ganhar da mesma pessoa duas vezes seguidas, sem jogar com mais ninguém no meio, a segunda não vale gol e não conta nas {meta?.futprego?.maxGoalsPerHour ?? 10}. Contra alguém do seu time é amistoso: vale só o dinheiro, sem gol e sem ponto no ranking. As outras partidas de verdade contam no Ranking X1 (rodada, temporada e geral, com prêmios): vitória +{meta?.futprego?.points?.win ?? 3}, empate +{meta?.futprego?.points?.draw ?? 1}, derrota {meta?.futprego?.points?.loss ?? -2}. Quem não é VIP espera {Math.round((meta?.futprego?.challengeCooldownSec ?? 120) / 60)} minutos depois de cada partida para desafiar de novo (aceitar desafio pode na hora); VIP joga o X1 sem esperar. Duas contas na mesma internet não se enfrentam. Libera para todos.</li>
