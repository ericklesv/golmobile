    ...
    <div className="flex flex-col gap-4">
      ...
      <Panel title="OS CHUTES" ribbon="orange">
        <ul className="flex flex-col gap-2 text-[13px] font-bold text-navy-ink">
          ...
          <li><b className="text-orange-deep">X1</b> — um jogo 1x1 ao vivo por dia, que troca à meia-noite: <b>FutPrego</b> (futebol de prego, uma vez de cada: arraste para trás e solte, e a bola quica nos pregos; sem gol em {meta?.futprego?.maxTurns ?? 10} jogadas de cada, o dinheiro volta) e <b>Futebol de Botão</b> (na sua vez, {meta?.x1?.botao.snapsPerTurn ?? 2} petelecos num botão seu; o primeiro gol acaba a partida; sem gol em {meta?.x1?.botao.maxTurns ?? 9} vezes, pênaltis). Quem desafia espera alguém aceitar (quem está nas telas do jogo recebe o convite). Cada um põe {fmt(meta?.futprego?.bet ?? 200)}; quem vencer leva {fmt((meta?.futprego?.bet ?? 200) * 2)} e 1 gol para o time, e o time do outro perde 1 gol na rodada. Até {meta?.futprego?.maxGoalWinsPerDay ?? 3} gols por dia; ganhar da mesma pessoa duas vezes seguidas, a segunda não vale gol. As vitórias que valem gol contam no Ranking do X1 da temporada. Times e internets diferentes. Libera para todos.</li>
