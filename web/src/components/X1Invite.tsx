...
interface Invite { id: number; gameName: string; from: { nick: string; avatarUrl: string | null; team: Team }; bet: number; seconds: number; until: number; sameTeam: boolean; freeplay: boolean }
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onmessage = (ev) => {
        ...
        if (m.t === 'invite') { setInv({ id: m.id, gameName: m.gameName ?? 'X1', from: m.from, bet: m.bet, seconds: m.seconds, until: Date.now() + m.seconds * 1000, sameTeam: !!m.sameTeam, freeplay: !!m.freeplay }); sound.play('pop'); }
        ...
        {inv && (
          <motion.div key={inv.id} role="alert" initial={{ y: -24, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -16, opacity: 0 }} transition={{ type: 'spring', stiffness: 380, damping: 28 }}
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[11px] font-bold text-muted">{inv.freeplay ? 'Vocês estão na mesma internet: é treino — sem aposta, sem gol e fora do Ranking X1.' : inv.sameTeam ? `Amistoso do seu time: quem vencer leva os ${fmt(inv.bet * 2)}, sem gol.` : `Cada um põe ${fmt(inv.bet)}; quem vencer leva tudo.`}</div>
